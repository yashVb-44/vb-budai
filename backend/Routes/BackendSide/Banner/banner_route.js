...
route.get('/get', checkAdminOrRole5, async (req, res) => {
    try {
        const banner = await Banner.find().populate('CategoryId', 'Category_Name').sort({ createdAt: -1 });
        ...
        if (banner) {
            ...
            const result = banner.map(banner => ({
                ...
                Category_Name: banner.CategoryId?.Category_Name
            }));
...
route.get('/mob/get', async (req, res) => {
    try {
        const banner = await Banner.find({ Banner_Status: true }).populate('CategoryId', 'Category_Name').sort({ createdAt: -1 });
        if (!banner) {
            res.status(404).json({ type: "warning", message: "No Banner found!", banner: [] })
        }
        else {
            const result = banner.map(banner => ({
                ...
                banner_link: banner.Banner_Link || "",
                categoryId: banner.CategoryId?._id,
                categoryName: banner.CategoryId?.Category_Name
            }));
            res.status(201).json({ type: "success", message: " Banner found successfully!", banner: result })
        }
    } catch (error) {
    ...
    try {
        ...
        if (!banner) {
            ...
            const result = {
                banner_id: banner._id,
                banner_Name: banner.Banner_Name,
                banner_Image: `http://${process.env.IP_ADDRESS}:${process.env.PORT}/${banner.Banner_Image?.path?.replace(/\\/g, '/')}` || "",
                banner_sequence: banner.Banner_Sequence,
                banner_link: banner.Banner_Link || "",
                categoryId: banner.CategoryId
            };
...
route.patch('/update/:id', checkAdminOrRole5, upload.single('image'), async (req, res) => {
    const bannerId = req.params.id;
    const { Banner_Name, Banner_Link, Banner_Sequence, Banner_Label, CategoryId } = req.body;
